  ...
  void print_coeffs(const int iter) {
    ...
    if (rank_ == 0) {
      double alpha =
          j_["bicgstab"][std::to_string(step_)][std::to_string(iter)]["alpha"];
      double rho =
          j_["bicgstab"][std::to_string(step_)][std::to_string(iter)]["rho"];
      double beta =
...
public:
  ...
  void log_vec(cudaStream_t solver_stream, const int iter,
    ...
    checkCudaErrors(cudaMemcpyAsync(vec.data(), d_vec, m * sizeof(double),
                                    cudaMemcpyDeviceToHost, solver_stream));
  ...
  void print_vec(const std::string tag, const int len, const int offset,
    ...
    checkCudaErrors(cudaMemcpyAsync(vec.data(), &d_vec[offset],
                                    len * sizeof(double),
                                    cudaMemcpyDeviceToHost, solver_stream));
    checkCudaErrors(cudaStreamSynchronize(solver_stream));
