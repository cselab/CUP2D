...
#include "PutObjectsOnGridStaggered.h"
#include "../Shape.h"
...
void PutObjectsOnGridStaggered::putChiOnGrid(Shape * const shape) const
{
  ...
  const auto chi =[&](const double dist,const double distPx,const double distMx,
                                        const double distPy,const double distMy)
  {
    if (dist > +2*h || dist < -2*h) return dist>0 ? 1.0 : 0.0;
    else {
      ...
      return (gradIX*gradUX + gradIY*gradUY) * denum;
    }
  };
  const auto chi_cellCenter = [&] (const CHI_MAT& sdf, const ScalarLab& SDIST,
                                   const int ix, const int iy)
  {
    ...
    return chi(sdf[iy][ix], distPx, distMx, distPy, distMy);
  };
  const auto chi_faceX = [&] (const CHI_MAT& sdf, const ScalarLab& SD,
                              const int ix, const int iy)
  {
    const auto distMx = ix == 0? SD(ix-1,iy).s : sdf[iy][ix-1];
    const auto distPx = sdf[iy][ix];
    const auto dist   = ( sdf[iy][ix] + distMx ) / 2;
    const auto distNC = iy+1==_BS_          ? SD(ix  ,iy+1).s : sdf[iy+1][ix  ];
    const auto distNW = iy+1==_BS_ || ix==0 ? SD(ix-1,iy+1).s : sdf[iy+1][ix-1];
    const auto distSC = iy  ==0             ? SD(ix  ,iy-1).s : sdf[iy-1][ix  ];
    const auto distSW = iy  ==0    || ix==0 ? SD(ix-1,iy-1).s : sdf[iy-1][ix-1];
    const auto distPy = (2*dist + distNC + distNW) / 4;
    const auto distMy = (2*dist + distSC + distSW) / 4;
    return chi(dist, distPx, distMx, distPy, distMy);
  };
  const auto chi_faceY = [&] (const CHI_MAT& sdf, const ScalarLab& SD,
                              const int ix, const int iy)
  {
    const auto distMy = iy == 0? SD(ix,iy-1).s : sdf[iy-1][ix];
    const auto distPy = sdf[iy][ix];
    const auto dist   = ( distPy + distMy ) / 2;
    const auto distEC = ix+1==_BS_          ? SD(ix+1,iy  ).s : sdf[iy  ][ix+1];
    const auto distES = ix+1==_BS_ || iy==0 ? SD(ix+1,iy-1).s : sdf[iy-1][ix+1];
    const auto distWC = ix  ==0             ? SD(ix-1,iy  ).s : sdf[iy  ][ix-1];
    const auto distWS = ix  ==0    || iy==0 ? SD(ix-1,iy-1).s : sdf[iy-1][ix-1];
    const auto distPx = (2*dist + distEC + distES) / 4;
    const auto distMx = (2*dist + distWC + distWS) / 4;
    return chi(dist, distPx, distMx, distPy, distMy);
  };
  ...
  #pragma omp parallel reduction(+ : _x, _y, _m)
  {
    ...
    for (size_t i=0; i < Nblocks; i++)
    {
      ...
      {
        o.chi [iy][ix] = chi_cellCenter(o.dist, distlab, ix, iy);
        o.chiX[iy][ix] = chi_faceX(o.dist, distlab, ix, iy);
        o.chiY[iy][ix] = chi_faceY(o.dist, distlab, ix, iy);
