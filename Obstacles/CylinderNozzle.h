...
class CylinderNozzle : public Shape {
  ...
  std::vector<Real>
      actuators_prev_value; // the value of each actuator at the previous step
  std::vector<Real>
      actuators_next_value;  // the value each actuator will obtain next
  const int Nactuators;      // total number of actuators
  const Real actuator_theta; // circular arc (in deg) of each actuator
  Real fx_integral =
      0; // integral of force in x acting on the cylinder for the time it takes
         // to switch from actuators_prev_value to actuators_next_value
  std::vector<Schedulers::ParameterSchedulerScalar>
      actuatorSchedulers; // object that interpolates in time from
                          // actuators_prev_value to actuators_next_value
                          // (actuator values change smoothly over time)
  Real t_change = 0; // used to record when actuator values change
  const Real
      regularizer; // used to regularize the reward in Reinforcement Learning
  const Real
      ccoef; // max actuation velocity allowed in ccoef * cylinder velocity
...
public:
  ...
  // this will introduce a small distrurbance in the cylinder velocity, which
  // helps create an assymetric wake with vortex shedding
  void updateVelocity(Real dt) override {
    Shape::updateVelocity(dt);
    constexpr Real t1 = 0.25;
    constexpr Real t2 = 0.50;
    omega =
        (sim.time > t1 && sim.time < t2)
