...
#pragma omp parallel
  {
    // In the case of the waterturbine we have 3 ellipses that are not centered
    // at 0

    // Define the center of blade relative to coordinate origin at T=0, written
    // as {x,y} with standard coordinate system Distance from coordinate origin
    // to blade (6cm) = 4 length of semi major axis (smajax) of ellipse (3cm)
    // Define orientation of ellipse, witten as (orientation + angle of blade)
    // in Fillbock line
    //------------------------------------------------------------------------------------------------------------------------
    ...
    // BLADE 2 (top right, moving from alinged with flow towards being
    ...
    // BLADE 3 (bottom right, moving from being perpendicular to towards being
    // aligned with flow)
