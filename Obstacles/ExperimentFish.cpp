...
ExperimentDataFish::loadFile(const std::string path) {
  ...
  if (myfile.is_open()) {
    ...
    std::cout << "[ExperimentFish] Unable to open center of mass file " << path
              << std::endl;
    fflush(0);
    abort();
  }
  ...
  if (t >= tNext) {
    ...
    if (t >= timeStart) {
      u = (centerOfMassData[idxNext][0] - centerOfMassData[idxLast][0]) /
          dtDataset;
      v = (centerOfMassData[idxNext][1] - centerOfMassData[idxLast][1]) /
          dtDataset;
      omega = ((centerOfMassData[idxNext][2] - centerOfMassData[idxLast][2]) /
               dtDataset) *
              (M_PI / 180);
    }
    ...
    midlineScheduler.transition(t, tLast, tNext, lastMidlineValues,
                                nextMidlineValues);
  }
