...
Naca::Naca(SimulationData &s, ArgumentParser &p, Real C[2]) : Fish(s, p, C) {
  ...
  if (sim.rank == 0 && sim.verbose)
    printf("[CUP2D] - NacaData Nm=%d L=%f t=%f A=%f w=%f xvel=%f yvel=%f "
...
void Naca::updateVelocity(Real dt) {
  ...
  if (sim.time < tAccel) {
    ...
    v = (1.0 - sim.time / tAccel) * 0.01 * forcedv +
...
void Naca::updateLabVelocity(int nSum[2], Real uSum[2]) {
  ...
  if (bFixedx) {
    (nSum[0])++;
    if (sim.time < tAccel)
      uSum[0] -= (1.0 - sim.time / tAccel) * 0.01 * forcedu +
                 (sim.time / tAccel) * forcedu;
    else
      uSum[0] -= forcedu;
  }
  if (bFixedy) {
    (nSum[1])++;
    if (sim.time < tAccel)
      uSum[1] -= (1.0 - sim.time / tAccel) * 0.01 * forcedv +
                 (sim.time / tAccel) * forcedv + v_heave;
    else
      uSum[1] -= forcedv + v_heave;
  }
}
