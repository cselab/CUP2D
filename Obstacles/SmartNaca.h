...
class SmartNaca : public Naca {
  ...
  const Real actuator_ds; // length of each actuator (as a fraction of the
                          // airfoil's cord/length: 0 < actuator_ds < 1)
  ...
  // smoothly interpolate from the previous actuation values
  // (actuators_prev_value) to the next actuation values (actuators_next_value).
  // We also use 't_change' to record when the change of actuation strengths
  // happened.
  ...
  Real fx_integral = 0; // time integral of x-force (drag) of airfoil, used when
                        // computing the RL reward
...
public:
  ...
  // Called when an RL action should be taken, in order to change actuation
  // velocities.
