...
// This is called after every timestep. It will impose the actuator velocities
// on the cylinder.
void CylinderNozzle::finalize() {
  // Step 1: Interpolate between previous values (actuators_prev_value) and next
  // values (actuators_next_value)
  ...
  // Step 2: For the current actuator values, loop over all grid points and
  // impose actuation velocities.
  ...
  const Real Uact_max =
      ccoef *
      pow(u * u + v * v,
          0.5); // Max actuation velocity as fraction of total cylinder velocity
  ...
  for (size_t i = 0; i < vInfo.size(); i++) {
    ...
    if (obstacleBlocks[info.blockID] == nullptr)
      continue; // if this block does not contain the cylinder, move to next
                // block

    // Get array with velocities that will be imposed (imposed actuation
    // velocities will be put here)
    ...
    for (int iy = 0; iy < ScalarBlock::sizeY; iy++)
      for (int ix = 0; ix < ScalarBlock::sizeX; ix++) {
        ...
        // ii. Get position (x,y) of the current grid point and make cylinder
        // center the origin of the coordinate system used.
        Real p[2];
        info.pos(p, ix, iy);
        const Real x = p[0] - Cx;
        const Real y = p[1] - Cy;

        // iii. If the current point is more than 2h (h:local grid spacing)
        // distance away from the cylinder surface, do nothing and check next
        // point.
        ...
        // vi. If current point lies with the circular arc of the closest
        // actuator, impose actuation velocity
        const Real theta0 = idx * dtheta; // center of closest actuator
        const Real phi = theta - theta0;  // distance (angle) of current point
                                         // from center of closest actuator
        if (std::fabs(phi) < 0.5 * actuator_theta ||
            (idx == 0 && std::fabs(phi - 2 * M_PI) < 0.5 * actuator_theta)) {
          // point is within the actuator, impose x- and y-components of
          // actuation velocity
          const Real rr = radius / pow(r, 0.5);
          const Real ur =
              Uact_max * rr * actuators[idx] *
              cos(M_PI * phi / actuator_theta); // actuation velocity magnitude
          UDEF[iy][ix][0] =
              ur * cos(theta); // x-component, projected using cylinder normal
                               // vector n = [cos(theta),sin(theta)]
          UDEF[iy][ix][1] =
              ur * sin(theta); // y-component, projected using cylinder normal
                               // vector n = [cos(theta),sin(theta)]
        }
      }
  }
...
void CylinderNozzle::act(std::vector<Real> action, const int agentID) {
  t_change =
      sim.time; // keep track of the current time, which is when the smooth
                // transition from current to new values will start.
  ...
  // The actuation velocities need to be bounded in [-ccoef*U,ccoef*U], where
  // U:cylinder velocity and they need to have a total mass flux of zero (i.e.:
  // surface integral of actuation velocity = 0) These conditions are equivalent
  // to having actions that are bounded in [-1,+1] and that have a sum of zero;
  // this is iteratively imposed in the following loop:
  bool bounded = false;
  while (bounded == false) {
    bounded = true;
    ...
    // ii. Substract the computed sum (mean) from all actions and then force
    // them to be in [-1,+1]
  ...
  // Now that the actions have zero mean and are bounded, set the previous and
  // the next actuation values that will be used for the smooth transition (in
  // 'finalize').
  for (size_t i = 0; i < action.size(); i++) {
