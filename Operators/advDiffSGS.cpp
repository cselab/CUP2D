...
static inline Real computeEddyViscosity(const Real C, const Real h,
  ...
  const Real dudxSq = dudx * dudx;
  const Real dudySq = dudy * dudy;
...
struct KernelAdvectDiffuseSGS {
  ...
  void operator()(VectorLab &lab, const BlockInfo &info) const {
    ...
    ScalarBlock &__restrict__ CS =
        *(ScalarBlock *)CsInfo[info.blockID].ptrBlock;
    ;
    for (int iy = 0; iy < VectorBlock::sizeY; ++iy)
      for (int ix = 0; ix < VectorBlock::sizeX; ++ix) {
        const Real C = CS(ix, iy).s;
        const std::array<Real, 2> dUV_adv_dif =
            d_adv_dif(lab, uinf, afac, dfac, ix, iy, h, sim.dt, C);
