    ...
    for (size_t i = 0; i < groups.size(); i++) {
      ...
      for (int gy = 0; gy < nYY / BS; gy++)
        for (int gx = 0; gx < nXX / BS; gx++) {
          ...
          s << "    <Topology TopologyType=\"3DCoRectMesh\" Dimensions=\" " << 2
            << "     " << 2 << "     " << 2 << "\"/>\n";
          s << "       <Geometry GeometryType=\"ORIGIN_DXDYDZ\">\n";
          s << "          <DataItem Dimensions=\"3\" NumberType=\"Double\" "
               "Precision=\"8\" "
               "Format=\"XML\">\n";
          s << "              " << std::scientific << group.oz << " "
            << group.oy + gy * BS * group.h << " "
            << group.ox + gx * BS * group.h << "\n";
          s << "          </DataItem>\n";
          s << "          <DataItem Dimensions=\"3\" NumberType=\"Double\" "
               "Precision=\"8\" "
               "Format=\"XML\">\n";
          s << "             " << std::scientific << BS * minh << " "
            << BS * group.h << " " << BS * group.h << "\n";
          s << "          </DataItem>\n";
          s << "       </Geometry>\n";
          s << "  </Grid>\n\n";
        }
...
void convert_to_uniform(std::string filename, int tttt) {
  ...
  for (long long i = my_start; i < my_end; i++) {
    ...
    points[0] = std::max(
        points[0],
        (long long)(allGroups[i].index[0] * BS + allGroups[i].nx) * aux);
    points[1] = std::max(
        points[1],
        (long long)(allGroups[i].index[1] * BS + allGroups[i].ny) * aux);
    points[2] = 1;
  }
...
#pragma omp parallel for
          ...
          for (int y_up = aux * y; y_up < aux * (y + 1); y_up++)
            for (int x_up = aux * x; x_up < aux * (x + 1); x_up++) {
              ...
              if (uniform_x >= my_start && uniform_x < my_end) {
                ...
                const int base_up = uniform_x - my_start +
                                    uniform_y * (my_end - my_start) +
                                    uniform_z * (my_end - my_start) * points[1];
  ...
  if (rank == 0) {
    ...
    s << "      <DataItem Dimensions=\"3\" NumberType=\"Double\" "
         "Precision=\"8\" "
         "Format=\"XML\">\n";
    s << "            " << std::scientific << Cfactor * minh << " "
      << Cfactor * minh << " " << Cfactor * minh << "\n";
    s << "       </DataItem>\n";
    s << "   </Geometry>\n";
    s << "   <Attribute Name=\"data\" AttributeType=\"" << "Scalar"
      << "\" Center=\"Cell\">\n";
    s << "      <DataItem ItemType=\"Uniform\"  Dimensions=\" " << 1 << " "
      << points[1] / Cfactor << " " << points[0] / Cfactor << " "
      << "\" NumberType=\"Float\" Precision=\" "
      << (int)sizeof(H5T_NATIVE_FLOAT) << "\" Format=\"HDF\">\n";
...
#pragma omp parallel for collapse(3)
    for (int z = 0; z < points[2]; z += Cfactor)
      for (int y = 0; y < points[1]; y += Cfactor)
        for (int x = 0; x < my_end - my_start; x += Cfactor) {
          const int i = (x / Cfactor) +
                        (y / Cfactor) * (my_end - my_start) / Cfactor +
                        (z / Cfactor) * (my_end - my_start) / Cfactor *
                            points[1] / Cfactor;
          const int base =
          ...
          for (int iy = 0; iy < Cfactor; iy++)
            for (int ix = 0; ix < Cfactor; ix++)
              uniform_grid_coarse[i] +=
                  uniform_grid[base + ix + iy * (my_end - my_start) +
                               iz * (my_end - my_start) * points[1]];
    ...
    fapl_id = H5Pcreate(H5P_FILE_ACCESS);
    H5Pset_fapl_mpio(fapl_id, MPI_COMM_WORLD, MPI_INFO_NULL);
    file_id = H5Fcreate((filename + "-uniform.h5").c_str(), H5F_ACC_TRUNC,
                        H5P_DEFAULT, fapl_id);
    H5Pclose(fapl_id);
...
#if Cfactor > 1
    H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, mspace_id, fspace_id, fapl_id,
             uniform_grid_coarse.data());
#else
    H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, mspace_id, fspace_id, fapl_id,
             uniform_grid.data());
#endif
  ...
  return;
  {
    ...
    for (int ir = 0; ir < Nr; ir++)
      for (int it = 0; it < Nt; it++) {
        ...
        for (long long iy = std::max(IY - 5, (long long)0);
             iy < std::min(IY + 5, points[1]); iy++)
          for (long long ix = std::max(IX - 5, (long long)my_start);
               ix < std::min(IX + 5, my_end); ix++) {
    ...
    if (rank == 0) {
      ...
      s << "      <DataItem Dimensions=\"3\" NumberType=\"Double\" "
           "Precision=\"8\" "
           "Format=\"XML\">\n";
      s << "            " << std::scientific << minh << " " << dt * r_min << " "
        << dr << "\n";
      s << "       </DataItem>\n";
      s << "   </Geometry>\n";
      s << "   <Attribute Name=\"data\" AttributeType=\"" << "Scalar"
        << "\" Center=\"Cell\">\n";
      s << "      <DataItem ItemType=\"Uniform\"  Dimensions=\" " << 1 << " "
        << Nt << " " << Nr << " " << "\" NumberType=\"Float\" Precision=\" "
        << (int)sizeof(H5T_NATIVE_FLOAT) << "\" Format=\"HDF\">\n";
      s << "       " << (filename + "-uniform-polar.h5").c_str() << ":/"
        << "data" << "\n";
      s << "     </DataItem>\n";
      s << "   </Attribute>\n";
      s << "  </Grid>\n\n";
