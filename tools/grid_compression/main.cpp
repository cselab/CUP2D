...
void convert_to_float(std::string filename, std::string gridname) {
  ...
  std::vector<short int> levels;
  {
    ...
    if (isInt) {
      ...
      H5Dread(dataset_id, dtype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              levels_int.data());
  ...
  for (size_t i = 0; i < blocks; i++) {
    ...
    for (int y = 0; y < ny; y += C)
      for (int x = 0; x < nx; x += C) {
        ...
        for (int j = 0; j < NCHANNELS; j++) {
          for (int yl = y; yl < y + C; yl++)
            for (int xl = x; xl < x + C; xl++) {
              element[j] +=
                  (float)amr[(i * nx * ny + yl * nx + xl) * NCHANNELS + j];
            }
          element[j] /= (C * C);
        ...
        const int bbase10 =
            (i * ny * nx + y * nx + x + C - 1) * ptsPerElement * dimension;
        const int bbase01 =
            (i * ny * nx + (y + C - 1) * nx + x) * ptsPerElement * dimension;
        const int bbase11 = (i * ny * nx + (y + C - 1) * nx + x + C - 1) *
    ...
    for (int y = 0; y < ny; y += ny)
      for (int x = 0; x < nx; x += nx) {
        const int bbase00 =
            (i * ny * nx + y * nx + x) * ptsPerElement * dimension;
        const int bbase10 =
            (i * ny * nx + y * nx + x + nx - 1) * ptsPerElement * dimension;
        const int bbase01 =
                             ...
                             H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  {
    ...
    hid_t dataset_id =
        H5Dcreate(file_id1, "data-c", H5T_NATIVE_FLOAT, fspace_id, H5P_DEFAULT,
                  H5P_DEFAULT, H5P_DEFAULT);
    H5Sclose(fspace_id);
    fspace_id = H5Dget_space(dataset_id);
    H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5P_DEFAULT, fspace_id, H5P_DEFAULT,
             data_c.data());
    H5Sclose(fspace_id);
    H5Dclose(dataset_id);
  }
  {
    ...
    hid_t fspace_id = H5Screate_simple(1, dims, NULL);
    hid_t dataset_id =
        H5Dcreate(file_id1, "vertices-c", H5T_NATIVE_FLOAT, fspace_id,
  ...
  }
  {
    ...
    hid_t fspace_id = H5Screate_simple(1, dims, NULL);
    hid_t dataset_id =
        H5Dcreate(file_id1, "vertices-grid", H5T_NATIVE_FLOAT, fspace_id,
                  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    H5Sclose(fspace_id);
    fspace_id = H5Dget_space(dataset_id);
    H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5P_DEFAULT, fspace_id, H5P_DEFAULT,
...

  {
    ...
    s << "        <DataItem ItemType=\"Uniform\"  Dimensions=\" "
      ...
      << "\" Format=\"HDF\">\n";
    s << "            " << (myfilename.str() + "-compressed.h5").c_str() << ":/"
      << "vertices-c" << "\n";
    s << "        </DataItem>\n";
    s << "     </Geometry>\n";
    if (NCHANNELS == 1)
...
int main(int argc, char **argv) {
  ...
  for (size_t f = 0; f < fields; f++) {
    ...
    for (size_t g = 0; g < gridnames.size(); g += size) {
      ...
      if (i + rank >= filenames.size())
        continue;
      if (g + rank >= gridnames.size())
        continue;
      std::cout << "converting " << filenames[i + rank] << ":" << i + rank
