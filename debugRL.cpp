 ...
 ************************
 This script can be used to reproduce a sample (simulation) that was performed
 with Korali during an RL training. It assumes that each agent has the same
 number of actions available to them (defined as 'NACTIONS').

 To reproduce the run that we want, we need:
  I. The initial conditions for that run.
  II.The actions taken by each agent.

 The initial conditions for the flow field are assumed to be the default ones
 from CUP2D. The initial conditions for the obstacles and the settings for the
 simulation need to be hardcoded in launch/debugRL.sh

 The actions taken by each agent need to be written in (and read from) files.
 The RL training should produce files named actions0.txt, actions1.txt,... (one
 for each agent). Each file contains (assuming NACTIONS=2) t0 a0 b0 t1 a1 b1 ...
 where a and b are the two actions and t is the time those two actions were
 taken.

 Once actions*.txt are produced (from the original RL run), they need to be
 copied in the directory where this executable will run. After copying them, use
 launch/debugRL.sh to launch the run.
*/
