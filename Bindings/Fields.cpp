...
struct BlockView {
  ...
  auto str() const {
    return py::str("BlockView({}x{} {}, ID={}, ij=({}, {}), level={}, Z={})")
        .format(BS, BS, isVector ? "vector" : "scalar", block->blockID,
                block->index[0], block->index[1], block->level, block->Z);
  }
  py::tuple ij() const {
  ...
  py::array_t<Real> toArray() const {
    ...
    return py::memoryview::from_buffer(
        ...
        isVector ? Vector{2 * BS * size, 2 * size, size}
                 : Vector{BS * size, size});
  }
  ...
  int level() const { return block->level; }
...
template <typename Grid> struct GridBlocksView {
  Grid *grid;
  size_t numBlocks() const { return grid->getBlocksInfo().size(); }
...
template <typename Grid>
static void gridLoadUniform(Grid *grid,
                            py::array_t<Real, py::array::c_style> array) {
  static constexpr bool kIsVector = std::is_same_v<Grid, VectorGrid>;
  using T = typename Grid::BlockType::ElementType;
  static_assert(sizeof(T) == (kIsVector ? 2 : 1) * sizeof(Real), "");
  const auto cells = grid->getMaxMostRefinedCells();
  const bool ok = array.ndim() == (kIsVector ? 3 : 2) &&
                  array.shape(0) == cells[1] && array.shape(1) == cells[0] &&
