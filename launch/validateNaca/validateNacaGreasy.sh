...
[@ ./ @] export BPDX=$i; export BPDY=$(($i / 2)); export ANGLE=14; export FPITCH=0; export EXTENT=1; export XPOS=0.2; ./launchNaca.sh NACA-bpdx=$i
EOF
done
...
[@ ./ @] export BPDX=$(( $i * 48)); export BPDY=$(( $i * 24)); export ANGLE=13.15; export FPITCH=0; export EXTENT=$i; export XPOS=$(( $i / 5 )); ./launchNaca.sh NACA-extent=$i
EOF
done
