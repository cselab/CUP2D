...
void AMRSolver::getZ(Real *input, BlockInfo &zInfo) {
...

  {
    ...
    const int tid = omp_get_thread_num();
    {
      ...
      // This is done to preserve symmetries. All it does is change the order
      // with which the elements of z(x,y) are accessed (x=0,...,N-1 or
      // x=N-1,...0, same for y)
...
void AMRSolver::solve(const ScalarGrid *input, ScalarGrid *const output) {
  ...
  const auto &zInfo =
      output->getBlocksInfo(); // will store the input 'x' when LHS is computed
  ...
  const size_t N = BSX * BSY * Nblocks; // total number of variables of this
                                        // rank
  const Real eps = 1e-100; // used in denominators, to not divide by zero
  const Real max_error =
      sim.step < 10
          ? 0.0
          : sim.PoissonTol; // error tolerance for Linf norm of residual
  const Real max_rel_error =
      sim.step < 10
          ? 0.0
          : sim.PoissonTolRel; // relative error tolerance for Linf(r)/Linf(r0)
  const int max_restarts =
      sim.step < 10 ? 100 : sim.maxPoissonRestarts; // maximum restarts allowed
